...
- **nix-removal v0.3 (2026-04-14):** Relax `[profile.release]` in `Cargo.toml` from `lto = true` (fat) to `lto = "thin"` — `make build-club` was OOM-killed at the final rustc link step under Colima's default 3.8 GiB VM; thin LTO preserves most optimization, keeps `codegen-units = 1`, and the image builds in ~104s.
- **nix-removal v0.3 (2026-04-14):** Fix `infra/docker/Dockerfile.club` and `infra/docker/Dockerfile.keybox`: add `RUN find crates -name '*.rs' -exec touch {} +` after the `COPY crates/ crates/` step. Docker COPY preserves host file mtimes, so cargo's incremental fingerprint check saw real source as older than the stub rlibs produced by the dep-cache layer and linked `moto-club` / `moto-keybox-server` against the stubs. Symptom: `error[E0432]: unresolved import 'moto_club_types::GarageId'` from `moto-club-k8s` while `moto-club-types` emitted stub-only `missing_docs` warnings. The yesterday-marked validation entry had recorded completion without actually running a clean club build to the end.
- **nix-removal v0.3 (2026-04-13):** Fix Docker layer caching in CI `build-images` job: switch buildx from `docker` driver to `docker-container` driver, pass moto-bike image via `--build-context` or temporary registry instead of `docker load`, then enable `--cache-to type=local` and `--cache-from type=local`
- **nix-removal v0.3 (2026-04-13):** Validate the fixes by running `make build-club && make build-keybox` end-to-end on a clean cache.
...
- **nix-removal v0.2 (2026-04-13):** Verify rustfmt and clippy installation in garage container: build Dockerfile.garage and test `cargo fmt --version && cargo clippy --version` inside. If they fail, add `rust-rustfmt` and `rust-clippy` apk packages. If they succeed, update dev-container.md spec table (lines 89-90) to document they are bundled with rust.
...
- **docs v0.2 (2026-03-13):** Fix `docs/getting-started.md` port override env vars (line 279): change `CLUB_PORT=28080 KEYBOX_PORT=29090 AI_PROXY_PORT=27070` to use actual env var names `MOTO_CLUB_BIND_ADDR`, `MOTO_KEYBOX_BIND_ADDR`, `MOTO_AI_PROXY_BIND_ADDR` with full address values — the documented vars don't exist and are silently ignored.
