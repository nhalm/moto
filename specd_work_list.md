...
- Fix keybox `audit_auth_failed` in `crates/moto-keybox/src/pg_api.rs` to store the failure reason in `metadata` (e.g. `{"reason": "..."}`) instead of `resource_id`; set `resource_id` to empty string or omit
