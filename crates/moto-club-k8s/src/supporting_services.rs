...
pub const REDIS_DEPLOYMENT_NAME: &str = "redis";
/// Redis Service name.
pub const REDIS_SERVICE_NAME: &str = "redis";
/// Redis credentials Secret name.
pub const REDIS_CREDENTIALS_SECRET_NAME: &str = "redis-credentials";
/// Redis port.
pub const REDIS_PORT: i32 = 6379;
/// Redis image.
const REDIS_IMAGE: &str = "redis:7";
...
pub trait GarageRedisOps {
    /// Creates Redis Deployment, Service, and credentials Secret.
    ///
    /// Per supporting-services.md spec, this creates:
    /// - Deployment with redis:7 image, readiness probe, resource limits
    /// - Service exposing port 6379
    /// - Secret with credentials (password, host, port, url)
    ///
    /// # Errors
    ///
    /// Returns an error if any K8s resource creation fails.
    fn create_garage_redis(&self, id: &GarageId) -> impl Future<Output = Result<()>> + Send;
...
impl GarageRedisOps for GarageK8s {
    #[instrument(skip(self), fields(garage_id = %id))]
    async fn create_garage_redis(&self, id: &GarageId) -> Result<()> {
        let namespace = format!("moto-garage-{}", id.short());
        ...
        // Create credentials Secret first (Deployment references it)
        debug!(namespace = %namespace, "creating Redis credentials Secret");
        let secret = build_redis_credentials_secret(&namespace, &password);
        let secret_api: Api<Secret> = Api::namespaced(self.client.inner().clone(), &namespace);
        secret_api
            .create(&PostParams::default(), &secret)
            .await
        ...
        // Create Deployment
        debug!(namespace = %namespace, "creating Redis Deployment");
        let deployment = build_redis_deployment(&namespace);
        let deployment_api: Api<Deployment> =
            Api::namespaced(self.client.inner().clone(), &namespace);
        deployment_api
            .create(&PostParams::default(), &deployment)
        ...
        // Create Service
        debug!(namespace = %namespace, "creating Redis Service");
        let service = build_redis_service(&namespace);
        let service_api: Api<Service> = Api::namespaced(self.client.inner().clone(), &namespace);
        service_api
            .create(&PostParams::default(), &service)
            .await
        ...
        debug!(namespace = %namespace, "Redis supporting service created");
        Ok(())
    }

    #[instrument(skip(self), fields(garage_id = %id))]
    async fn redis_exists(&self, id: &GarageId) -> Result<bool> {
        let namespace = format!("moto-garage-{}", id.short());
        let api: Api<Deployment> = Api::namespaced(self.client.inner().clone(), &namespace);
...
/// Builds Redis credentials Secret per spec.
fn build_redis_credentials_secret(namespace: &str, password: &str) -> Secret {
    let mut labels = BTreeMap::new();
    labels.insert("app".to_string(), REDIS_DEPLOYMENT_NAME.to_string());
    labels.insert(SUPPORTING_SERVICE_LABEL.to_string(), "true".to_string());

    // URL format per spec: redis://:{password}@redis:6379
    let url = format!("redis://:{password}@{REDIS_SERVICE_NAME}:{REDIS_PORT}");

    let mut string_data = BTreeMap::new();
    string_data.insert("password".to_string(), password.to_string());
    string_data.insert("host".to_string(), REDIS_SERVICE_NAME.to_string());
    string_data.insert("port".to_string(), REDIS_PORT.to_string());
    string_data.insert("url".to_string(), url);

    Secret {
        metadata: ObjectMeta {
            name: Some(REDIS_CREDENTIALS_SECRET_NAME.to_string()),
            namespace: Some(namespace.to_string()),
            labels: Some(labels),
            ..Default::default()
        },
...
/// Builds Redis Deployment per spec.
#[allow(clippy::too_many_lines)]
fn build_redis_deployment(namespace: &str) -> Deployment {
    let mut labels = BTreeMap::new();
    labels.insert("app".to_string(), REDIS_DEPLOYMENT_NAME.to_string());
    labels.insert(SUPPORTING_SERVICE_LABEL.to_string(), "true".to_string());
    ...
    // Environment variable for password (used by args and readiness probe)
    ...
    // Readiness probe per spec: redis-cli -a $(REDIS_PASSWORD) ping
    let readiness_probe = Probe {
        exec: Some(ExecAction {
            command: Some(vec![
                "sh".to_string(),
                "-c".to_string(),
                "redis-cli -a $REDIS_PASSWORD ping".to_string(),
            ]),
        }),
        initial_delay_seconds: Some(5),
        period_seconds: Some(5),
    ...
    let container = Container {
        name: REDIS_DEPLOYMENT_NAME.to_string(),
        image: Some(REDIS_IMAGE.to_string()),
        ports: Some(vec![ContainerPort {
            container_port: REDIS_PORT,
            ..Default::default()
        }]),
        // Args per spec: --requirepass $(REDIS_PASSWORD)
        // Note: Shell expansion of env var happens because we use sh -c in args
        args: Some(vec![
            "sh".to_string(),
            "-c".to_string(),
            "redis-server --requirepass $REDIS_PASSWORD".to_string(),
        ]),
        env: Some(env_vars),
        resources: Some(resources),
        volume_mounts: Some(volume_mounts),
    ...
    Deployment {
        metadata: ObjectMeta {
            name: Some(REDIS_DEPLOYMENT_NAME.to_string()),
            namespace: Some(namespace.to_string()),
            labels: Some(labels.clone()),
            ..Default::default()
        },
        spec: Some(DeploymentSpec {
            ...
            selector: LabelSelector {
                match_labels: Some({
                    let mut selector = BTreeMap::new();
                    selector.insert("app".to_string(), REDIS_DEPLOYMENT_NAME.to_string());
                    selector
                }),
                ..Default::default()
            },
...
/// Builds Redis Service per spec.
fn build_redis_service(namespace: &str) -> Service {
    let mut labels = BTreeMap::new();
    labels.insert("app".to_string(), REDIS_SERVICE_NAME.to_string());
    labels.insert(SUPPORTING_SERVICE_LABEL.to_string(), "true".to_string());
    ...
    Service {
        metadata: ObjectMeta {
            name: Some(REDIS_SERVICE_NAME.to_string()),
            namespace: Some(namespace.to_string()),
            labels: Some(labels),
            ..Default::default()
        },
...
mod tests {
    ...
    #[test]
    fn build_redis_credentials_secret_structure() {
        let secret = build_redis_credentials_secret("moto-garage-abc12345", "testpass123");

        // Check metadata
        assert_eq!(
            secret.metadata.name,
            Some(REDIS_CREDENTIALS_SECRET_NAME.to_string())
        );
        assert_eq!(
            secret.metadata.namespace,
            Some("moto-garage-abc12345".to_string())
        ...
        // Check labels
        let labels = secret.metadata.labels.as_ref().unwrap();
        assert_eq!(labels.get("app"), Some(&REDIS_DEPLOYMENT_NAME.to_string()));
        assert_eq!(
            labels.get(SUPPORTING_SERVICE_LABEL),
            Some(&"true".to_string())
        );
        ...
        let data = secret.string_data.as_ref().unwrap();
        assert_eq!(data.get("password"), Some(&"testpass123".to_string()));
        assert_eq!(data.get("host"), Some(&REDIS_SERVICE_NAME.to_string()));
        assert_eq!(data.get("port"), Some(&"6379".to_string()));
        assert_eq!(
            data.get("url"),
            Some(&"redis://:testpass123@redis:6379".to_string())
        );
    }

    #[test]
    fn build_redis_deployment_structure() {
        let deployment = build_redis_deployment("moto-garage-abc12345");

        // Check metadata
        assert_eq!(
            deployment.metadata.name,
            Some(REDIS_DEPLOYMENT_NAME.to_string())
        );
        assert_eq!(
            deployment.metadata.namespace,
            Some("moto-garage-abc12345".to_string())
        ...
        let container = &pod_spec.containers[0];
        assert_eq!(container.name, REDIS_DEPLOYMENT_NAME);
        assert_eq!(container.image, Some(REDIS_IMAGE.to_string()));

        // Check ports
        let ports = container.ports.as_ref().unwrap();
        assert_eq!(ports.len(), 1);
        assert_eq!(ports[0].container_port, REDIS_PORT);

        // Check env vars
        let env = container.env.as_ref().unwrap();
        let pass_env = env.iter().find(|e| e.name == "REDIS_PASSWORD").unwrap();
        let secret_ref = pass_env
            .value_from
            .as_ref()
            .unwrap()
        ...
        // Check args (--requirepass via shell)
        let args = container.args.as_ref().unwrap();
        assert!(args.contains(&"redis-server --requirepass $REDIS_PASSWORD".to_string()));
        ...
        // Check readiness probe (redis-cli ping via shell)
        let probe = container.readiness_probe.as_ref().unwrap();
        let exec = probe.exec.as_ref().unwrap();
        let command = exec.command.as_ref().unwrap();
        assert!(command.contains(&"redis-cli -a $REDIS_PASSWORD ping".to_string()));
        assert_eq!(probe.initial_delay_seconds, Some(5));
        assert_eq!(probe.period_seconds, Some(5));
    ...
    #[test]
    fn build_redis_service_structure() {
        let service = build_redis_service("moto-garage-abc12345");

        // Check metadata
        assert_eq!(service.metadata.name, Some(REDIS_SERVICE_NAME.to_string()));
        assert_eq!(
            service.metadata.namespace,
            Some("moto-garage-abc12345".to_string())
        );
        ...
        // Check labels
        let labels = service.metadata.labels.as_ref().unwrap();
        assert_eq!(labels.get("app"), Some(&REDIS_SERVICE_NAME.to_string()));
        assert_eq!(
            labels.get(SUPPORTING_SERVICE_LABEL),
            Some(&"true".to_string())
        );
        ...
        let ports = spec.ports.as_ref().unwrap();
        assert_eq!(ports.len(), 1);
        assert_eq!(ports[0].port, REDIS_PORT);
        assert_eq!(ports[0].target_port, Some(IntOrString::Int(REDIS_PORT)));
    }
}
